        ...
        for j, epoch in enumerate(range(epochs)):
            ...
            if acc > best_dev_acc:
                ...
                basedir = os.path.join("saved_models", "{0}-{1}".format(DATASET, FINE_GRAINED),
                                       AUXILIARY_TASK,
                                       "{0}".format(run + 1))
                outname = "epochs:{0}-lstm_dim:{1}-lstm_layers:{2}-devacc:{3:.3f}.pt".format(epoch + 1, model.lstm1.hidden_size, model.lstm1.num_layers, acc)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--EMBEDDINGS", "-emb",
                        default="../../embeddings/embeddings/google.txt")
